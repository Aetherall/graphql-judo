import { formatError, execute, subscribe, parse } from 'graphql';
...
class Judo {
  constructor(_config) {
    ...
    _defineProperty(this, "mergeTypeDefs", () => {
      return mergeTypes([this.config.graphqlFiles.datamodel, this.config.graphqlFiles.prisma, ...(this.config.graphqlFiles.typeDefs || [])], {
        all: true,
        override: true
      });
    });
    ...
    _defineProperty(this, "startHttpServer", () => {
      ...
      this.express.post(this.config.endpoint, bodyParserGraphQL()); //this.express.post(this.config.endpoint, apolloUploadExpress(this.config.uploads))
